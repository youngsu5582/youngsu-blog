...
const SLUG_VALIDATION_MESSAGE =
  "Slug는 한글/영문 소문자/숫자/하이픈만 사용할 수 있고, / 또는 ..은 사용할 수 없어요.";

function validateSlug(value: string) {
  if (!value) return null;
  if (value.includes("/") || value.includes("..") || /\s/.test(value))
    return SLUG_VALIDATION_MESSAGE;
...
export default function WritePage() {
  ...
  const [series, setSeries] = useState<SeriesFormValue>({
    series: "",
    seriesOrder: "",
    seriesDescription: "",
  ...
  const performAutoSave = useCallback(() => {
    ...
    const existingIndex = currentDrafts.findIndex(
      (d) => (slug && d.slug === slug) || (!slug && d.title === title),
    );
  ...
  const loadDraft = (draft: Draft) => {
    ...
    setSeriesMode(draft.seriesMode ?? (draft.series?.series ? "existing" : "none"));
    setSeries(
      draft.series ?? {
        series: "",
        seriesOrder: "",
  ...
  const commitTemplate = (templateId: string) => {
    ...
    setRelatedSlugs([]);
    setSeriesMode("none");
    setSeries({ series: "", seriesOrder: "", seriesDescription: "", seriesStatus: "ongoing" });
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/series")
      .then((r) => r.json())
      .then((data) => {
        if (data.series) setSeriesOptions(data.series);
      })
      .catch(() => {});
  }, []);
  ...
  const handleSave = async () => {
    ...
    if (slugError) {
      setResult({ success: false, message: slugError });
      return;
    }
    setSaving(true);
    setResult(null);
    try {
      const res = await fetch("/api/admin/write", {
        ...
        body: JSON.stringify({
          ...
          ...(collection === "posts" && seriesMode !== "none" ? series : {}),
        }),
      });
  ...
  const summarizeList = (values: string[]) =>
    values.length > 0 ? values.join(", ") : "(비어 있음)";
  ...
  return (
    <div className="space-y-4">
                    ...
                    <div className="py-1">
                      {drafts.map((draft) => {
                        const collectionLabel =
                          COLLECTIONS.find((c) => c.id === draft.collection)?.label ||
                          draft.collection;
                        return (
                          <div
                            key={draft.id}
      ...
      {pendingRestoreDraft && (
        ...
        >
          <div>
            저장된 임시글 <span className="font-medium">{pendingRestoreDraft.title}</span>이 있어요.
            현재 새 글을 덮어쓰지 않고 필요할 때만 복원할 수 있어요.
          </div>
        ...
        >
          <div>
            ...
            <p className="text-xs opacity-80 mt-1">
              현재 작성 중인 내용을 바로 덮어쓰지 않고 변경될 항목을 먼저 보여줘요.
            </p>
          </div>
          <div className="space-y-1 text-xs font-mono">
            <p>
              컬렉션: {collection} → {pendingTemplate.collection}
            </p>
            <p>
              카테고리: {summarizeList(categories)} → {summarizeList(pendingTemplate.categories)}
            </p>
            <p>
              본문: {summarizeText(content)} → {summarizeText(pendingTemplate.content)}
            </p>
          </div>
          <div className="flex gap-2">
            <button
        ...
        <div className="rounded-lg border border-border/60 p-4 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              />
              {slugError ? (
                <p id="write-slug-error" className="text-[10px] text-red-600 dark:text-red-400">
                  {slugError}
                </p>
              ) : (
                <p id="write-slug-preview" className="text-[10px] text-muted-foreground/50">
                  파일: content/{collection}/{slug || "..."}.mdx
                </p>
              )}
            </div>
          </div>
        ...
        <div className="flex flex-col overflow-hidden rounded-xl border border-border/60 bg-background shadow-sm transition-shadow focus-within:border-primary/40 focus-within:shadow-md">
          <div className="flex items-start justify-between gap-3 border-b border-border/40 bg-muted/30 px-3 py-2">
            ...
            <span className="rounded-full bg-background/70 px-2 py-0.5 text-[10px] text-muted-foreground/60 ring-1 ring-border/40">
              {content.length.toLocaleString()}자 · 약{" "}
              {Math.max(1, Math.ceil(content.length / 500))}분
            </span>
          </div>
          <MarkdownToolbar
