...
import { buildUploadPublicUrl, getUploadStorageConfig, resolveUploadObjectKey } from "@/lib/admin-upload-storage";
...
describe("admin upload storage config", () => {
  ...
  it("expands upload prefix variables and returns the Cloudflare URL", () => {
    ...
    expect(resolveUploadObjectKey("blog/${basename}", "123-image.png", now)).toBe("blog/123-image/123-image.png");
    expect(buildUploadPublicUrl("https://img.youngsu5582.today", "blog/2026/06/123-image.png")).toBe(
      "https://img.youngsu5582.today/blog/2026/06/123-image.png"
    );
  });
});
