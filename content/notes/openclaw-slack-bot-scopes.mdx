...
| `chat:read` | 메시지 읽기 (Events API 사용 시) |
...
### 스레드 (선택 - `chat:write`로 커버되지만 명시 권장)

| Scope | 설명 |
|-------|------|
| `chat:write` | 스레드 답글, 브로드캐스트 등 (스레드 접근은 `channels:history` / `groups:history`로 커버) |
...
- [OpenClaw Slack 설정 가이드](https://docs.openclaw.ai)
