---
title: Why an ECS Staging Server Was Redeployed Even Though It Was Healthy
date: 2026-06-28
description: A breakdown of why an ECS staging server kept getting redeployed after booting normally, focusing on healthCheckGracePeriodSeconds, container health checks, and ALB target health checks.
categories:
  - AWS
tags:
  - ecs
...
1. Just because an ECS task reaches `RUNNING` does not mean it is immediately ready to serve traffic.
2. In an ECS service behind a load balancer, both the Container Health Check and the ALB Target Health Check must pass before the task is considered stably healthy.
...
# Why an ECS Staging Server Was Redeployed Even Though It Was Healthy

Recently, I had a case where a staging server went down once and was redeployed again even though there was no obvious application error.

At first I thought, “Did the server actually die?” But after tracing the logs, it turned out that the app itself had not crashed abnormally.
The real issue was the timing of ECS and ALB health checks, along with the `healthCheckGracePeriodSeconds` setting.

In this post, I want to organize the settings I checked at that time and explain why the server was judged as a termination target even after it had booted normally.
...
And the process of a task becoming alive is a little different from the process of it becoming “actually ready to receive traffic.”
...
The important point here is that **`RUNNING` does not immediately mean the service is in a healthy serving state**.
...
In other words, it is checking whether the application is up from the perspective of `localhost:8080`.

The points that stood out again while reviewing this were these.

- Because it is an in-container command, it does not guarantee network reachability from outside or through the ALB path.
- On the other hand, it can keep failing until the app is fully up.
- If there is no `startPeriod`, failures during initial boot are still observed right away.
...
For example, if the traffic port is 28080, the ALB will send requests to `/api/health` on that port every 30 seconds to check the state.
...
The problem was that **the ALB Target Health Check does not start from the moment the app becomes ready. It starts from the moment the task enters `ACTIVATING`**.
...
- 44:31 server received termination request
- 44:35 server terminated

At first, it was also a bit confusing why there was about a one-minute gap between “server marked for termination” and “termination request sent.”
...
We need to give `healthCheckGracePeriodSeconds` more room.
...
The clearest way to think about it for me was like this.

- The ALB health check starts from `ACTIVATING`, regardless of whether the service is actually ready.
- Normal completion only happens after **app readiness time + ALB consecutive success time** have both passed.
...
and more about **not giving enough time for the server to be officially recognized as healthy**.
...
Looking at all four together can reveal the cause much faster than expected.
