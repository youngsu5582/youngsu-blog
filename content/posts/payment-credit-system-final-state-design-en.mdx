...
> The HTTP request is accepted quickly, while credit application is handled by a Kafka consumer. Credits and processing history are finalized in a database transaction, and the completion callback is sent after commit.
...
![Scope of idempotency, ordering, and the lock](/assets/img/diagrams/payment-credit-invariants.svg)

> Even when problems look similar, we use different mechanisms according to the invariant we need to protect.

The unit for each mechanism emerged naturally from the invariant it protected.
